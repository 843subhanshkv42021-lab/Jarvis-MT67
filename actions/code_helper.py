...
# ── FIXED: now uses google-genai (new SDK) instead of google.generativeai ──
...
def _generate(client_model_tuple, prompt: str) -> str:
    """Helper to generate content using new SDK."""
    from google.genai import types
    client, model = client_model_tuple
...
def _detect_intent(description: str, file_path: str, code: str) -> str:
    ...
    optimize_kw = ["optimize", "refactor", "clean up", "improve", "make it better"]
...
Code:"""
    ...
    code = _clean_code(_generate(cm, prompt))
...
Fixed code:"""
    ...
    return _clean_code(_generate(cm, prompt))
...
Updated code:"""
    ...
    try:
        edited = _clean_code(_generate(cm, prompt))
    except Exception as e:
...
Explanation:"""
    ...
    try:
        return _generate(cm, prompt).strip()
    except Exception as e:
...
Optimized code:"""
    ...
    try:
        optimized = _clean_code(_generate(cm, prompt))
    except Exception as e:
